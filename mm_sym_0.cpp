...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    int chunk = 1;
    printf("Starting parallel matrix multiplication C = C + A*A^T...\n\n");
    omp_set_num_threads(4);
    double tt = omp_get_wtime();
#pragma omp parallel
#pragma omp for private(j,k,ct0,ct1,ct2,ct3) schedule(dynamic,chunk)
    for (i = 0; i < NRA/4*4; i+=4)
        for (j = i; j < NRA; j++)
        {
            ct0 = c[i][j];
            ct1 = c[i + 1][j];
